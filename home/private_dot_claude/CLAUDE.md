...
- Pre-action research scales with the cost of being wrong: the
  ...
  before acting. A safety net (a backup, a saved state, an undo
  path) enables recovery but does not replace understanding the
  change.
...
Verify the outcome, not just the absence of errors — show evidence,
not assertions ("it works" is not verification). A passing check is
not the same as the problem being fixed. State the expected outcome
before observing the result — this prevents post-hoc rationalization
of whatever output appears.
...
- Diagnosing: before each investigation step, state the hypothesis,
...
- If two fix attempts on the same hypothesis fail, reassess: you
  ...
  experiments — or propose returning to the last working state as
  the fast path.
...
- What: state summaries for complex tasks; root cause + fix + pattern for
  failures/incidents/investigations; boundary adjustments that work.
...
Pasted terminal output without instructions: contains errors →
diagnose the root cause and propose a fix; shows success after a
prior task → acknowledge and proceed; unexpected state → explain
what happened.
...
- "didn't we already enable X?" → you missed existing state.
  Re-inspect the current state before responding.
...
Test: if this turns out wrong, can you undo it in one step
— and does the damage stop at the current workspace? If yes,
act without asking.
Even then, a choice embedded in the change (a fixed value, a
changed default, a tradeoff) gets a one-line flag — don't leave
it to be discovered in review.
...
A single correction adjusts the action; the same correction type
recurring — especially across sessions — means the governing rule
needs questioning: propose updating the rule itself. A loosened
boundary that causes an incident → revert and record why.
...
- Always create a worktree before writing code. Then PR →
  ...
  waiting on (review bots, deploy status) — don't wait to be
  asked → if CI fails, diagnose and fix. Before declaring a PR
  complete, verify it is mergeable against the target branch.
